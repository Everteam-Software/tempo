...
<intalio:loginBody subMenuHeader="Log In" logoPath="${logoPath}" scripts="${scripts}" pageTitle="${pageTitle}" footer="${footer}">
	<table width="350" border="0" cellspacing="4" cellpadding="0" >
			...
			<spring:bind path="login.username">
					...
					<c:choose>
						<c:when test="${status.error}">
							<label style="color:red">
								<fmt:message key="com_intalio_bpms_workflow_login_username"/>
							</label>
						</c:when>
						<c:otherwise>
							<label>
				...
				<td>
					<input type="text" name="username" 
						...
						class="textInput" style="width: 250px; " />
			...
			<spring:bind path="login.password">
				<td width="80" valign="top">
					<c:choose>
						<c:when test="${status.error}">
							<label style="color:red">
								<fmt:message key="com_intalio_bpms_workflow_login_password"/>
							</label>
						</c:when>
						<c:otherwise>
							<label>
		...
		<tr>
			...
			<td >
				...
				<fmt:message key="com_intalio_bpms_workflow_auto_login"/>
			</td>
		</tr>
		<tr>
			<td>&nbsp;</td>		
