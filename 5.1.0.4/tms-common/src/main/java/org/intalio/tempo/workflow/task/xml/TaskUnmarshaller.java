...
public class TaskUnmarshaller extends OMUnmarshaller {
    ...
    public Task unmarshalTaskFromMetadata(OMElement rootElement) throws InvalidInputFormatException {
        ...
        expectElementValue(rootQueue, "userProcessEndpoint"); // TODO: these violate the WSDL! do something
        expectElementValue(rootQueue, "userProcessNamespaceURI");
        String completeSOAPAction = expectElementValue(rootQueue, "userProcessCompleteSOAPAction");
        OMElement attachmentsElement = expectElement(rootQueue, "attachments");
        String isChainedBeforeStr = expectElementValue(rootQueue, "isChainedBefore");
        ...
        if (! ITaskWithState.class.isAssignableFrom(taskClass)) {
            ...
            try {
                taskState = (taskStateStr == null) ? TaskState.READY : TaskState.valueOf(taskStateStr
                        .toUpperCase());
        ...
        if (ITaskWithAttachments.class.isAssignableFrom(taskClass)) {
            ...
            if (attachmentsElement != null) {
                OMElementQueue attachmentQueue = new OMElementQueue(attachmentsElement);
                boolean done = false;
                while (! done) {
                    OMElement attachmentElement = expectElement(attachmentQueue, "attachment");
