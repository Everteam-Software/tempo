...
CREATE TABLE tempo_notification (id BIGINT NOT NULL, failure_code VARCHAR(254), failure_reason VARCHAR(254), input_xml CLOB(1M), instanceId VARCHAR(254), priority INTEGER, state SMALLINT, PRIMARY KEY (id));
CREATE TABLE tempo_pa (id BIGINT NOT NULL, complete_soap_action VARCHAR(254), deadline TIMESTAMP, failure_code VARCHAR(254), failure_reason VARCHAR(254), input_xml CLOB(1M), instance_id VARCHAR(254), is_chained_before SMALLINT, output_xml CLOB(1M), previous_task_id VARCHAR(254), priority INTEGER, process_id VARCHAR(254), state SMALLINT, PRIMARY KEY (id));
