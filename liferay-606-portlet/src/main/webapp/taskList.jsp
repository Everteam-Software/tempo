<%@ page contentType="text/xml; charset=UTF-8" language="java" import="java.sql.*,java.util.*,rokudo.sax.*,org.dom4j.*" errorPage="" %>
...
<%  ///////////////// RETRIEVE TOKEN
		String wsdlURLa = "http://"+intalioURL+"/axis2/services/TokenService.Service";
...
<% ////// RETRIEVE TASK LIST
		...
		String wsdlURL = "http://"+intalioURL+"/axis2/services/TaskManagementServices";
		...
		if (tipoList.equals("INIT")) {
			...
			req.addChild("subQuery", "tas").setText(" ORDER BY t._creationDate DESC");
			
		} else {
		
			req.addChild("subQuery", "tas").setText("T._state = TaskState.READY or T._state = TaskState.CLAIMED ORDER BY t._creationDate DESC");
		...
		for (STElement ste : list) {
			...
			if (nDescription.equals("")) { nDescription = "Empty Description"; }
			row.addElement("description").addText("<span class='des'>"+nDescription+"</span>");
			...
			for (STElement ste2 : listas) {	
				...
				nFile = ste2.getChild("attachmentMetadata").getChildContent("fileName");
				nURL =  ste2.getChildContent("payloadUrl");
				nName ="<img src='/image/image_gallery?uuid=802a6dbe-9c43-48c1-9937-0264a72a9a00&groupId=18&t=1344279777502' title='"+nFile+"' border='0' />";
			
				cdataAdd += "<div class='attachment'><a href='"+nURL+"' title='"+nFile+"' target='_BLANK'>"+nName+"</a></div>";
