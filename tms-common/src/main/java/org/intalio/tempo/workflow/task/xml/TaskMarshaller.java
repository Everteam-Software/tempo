...
public class TaskMarshaller {
	...
	final static String[] ACTIONS = new String[] { "claim", "revoke", "save",
			"complete", "dismiss" };
	...
	public OMElement marshalTaskMetadata(Task task, UserRoles roles) {
		OMElement om = XmlTooling.convertDocument(marshalXMLTaskMetadata(task,
				roles));
	...
	private XmlObject marshalXMLTaskMetadata(Task task, UserRoles roles) {
		...
		if (task instanceof InitTask) {
			...
			taskMetadataElement.setInitOperationSOAPAction(((InitTask) task)
		...
		if (task.getClass().equals(PATask.class)) {
			...
			ArrivalDepartureType arrivalDeparture = ArrivalDepartureType.Factory
					.newInstance();
			...
			AssignedMechanics[] assignedMechanicsArray = new AssignedMechanics[paTask
					.get_assignedMechanics().size()];
			org.intalio.tempo.workflow.task.AssignedMechanics[] mechanics = paTask
					.get_assignedMechanics()
					.toArray(
							new org.intalio.tempo.workflow.task.AssignedMechanics[0]);
			for (int i = 0; i < mechanics.length; i++) {
				assignedMechanicsArray[i] = AssignedMechanics.Factory
						.newInstance();
				// assignedMechanicsArray[i].setAssignedMechanicID(mechanics[i].getMechanicID());
				assignedMechanicsArray[i].setAssignedMechanicName(mechanics[i]
						.getName());
			}
	...
	private void createACL(String action, Task task, UserRoles roles,
		...
		if (task.getAuthorizedActions().contains(action)) {
			try {
				Method m = taskMetadata.getClass().getMethod(
						"addNew" + action.substring(0, 1).toUpperCase()
								+ action.substring(1) + "Action",
						(Class[]) null);
				AccessControlType ac = (AccessControlType) m.invoke(
				...
				// this should only happen if the method for setting the action
				// is not found in the taskMetadata class
				throw new RuntimeException(e);
			}
		}
	}

	public void marshalTaskInput(
			ITaskWithInput task,
	...
	static XmlObject extractXmlObject(final Document doc) throws XmlException {
		...
		try {
			...
			String xmlns = (dot > 0) ? "xmlns:" + nodeName.substring(0, dot)
					: "xmlns";
	...
	public void marshalTaskOutput(
			ITaskWithOutput task,
	...
	private void marshalFullTask(
			Task task,
			com.intalio.bpms.workflow.taskManagementServices20051109.Task parent,
			UserRoles user) {
		...
		if (task instanceof ITaskWithInput) {
			...
			com.intalio.bpms.workflow.taskManagementServices20051109.Task.Input taskInput = parent
					.addNewInput();
		...
		if (task instanceof ITaskWithOutput) {
			...
			if (taskWithOutput.getOutput() != null) {
				com.intalio.bpms.workflow.taskManagementServices20051109.Task.Output taskOutput = parent
						.addNewOutput();
