...
public class TaskFieldConverter {
    ...
    static {
        ...
        ROLES_MAP.put(GenericRoleType.excluded_owners.name(), "excludedOwners");
    ...
    public static String getQueryColumn(String viewField) throws InvalidFieldException {
        ...
        if (result == null) {
            result = SELECT_ROLES_MAP.get(viewField);
            if (result == null)
                throw new InvalidFieldException("Invalid field has been found in select clause: " + viewField);
        }
    ...
    public static ParameterValues convertWhereClause(String viewField, String funName, Object value) throws InvalidFieldException {
        ...
        } else if (TaskView.CREATED_ON.equals(viewField) || TaskView.ACTIVATION_TIME.equals(viewField) || TaskView.EXPIRATION_TIME.equals(viewField)
                        || TaskView.STARTBY.equals(viewField) || TaskView.COMPLETE_BY.equals(viewField)) {

            return convertDateField(mappingField, funName, value);

        } else if ((TaskView.STARTBYEXISTS.equals(viewField)) || (TaskView.COMPLETE_BY_EXISTS.equals(viewField))
                        || (TaskView.RENDER_METH_EXISTS.equals(viewField)) || (TaskView.HAS_POTENTIAL_OWNERS.equals(viewField))) {
    ...
    public static String getFieldForOrderClause(String field) throws InvalidFieldException {
        ...
        if (TaskView.HAS_POTENTIAL_OWNERS.equals(viewField) || (TaskView.FAULT_MESSAGE.equals(viewField)) || (TaskView.INPUT_MESSAGE).equals(viewField)
                        || (TaskView.OUTPUT_MESSAGE.equals(viewField)) || (TaskView.ATTACHMENT_NAME.equals(viewField))
                        || (TaskView.ATTACHMENT_TYPE.equals(viewField)) || (TaskView.USERID.equals(viewField)) || (TaskView.GROUP.equals(viewField))
        ...
        if (result == null) {
            throw new InvalidFieldException("Invalid field has been found in select clause: " + field);
        }

        return result;
    }

    private static ParameterValues convertTaskType(String fieldName, String funName, Object value) throws InvalidFieldException {
        ...
        } else if ((funName.equals(QueryOperator.NOT_EQUALS)) || (funName.equals(QueryOperator.NOT_EQUALS2))) {
            if (TaskQueryType.ALL.toString().equalsIgnoreCase(strValue)) {
                result = new ParameterValues();
    ...
    private static ParameterValues convertDateField(String fieldName, String funName, Object value) throws InvalidFieldException {
        // Date type
        Date date = null;
        try {
            date = QueryUtil.formatDate((String) value);
        } catch (ParseException e) {
            throw new InvalidFieldException("The date should be as the format like: '2009-02-27 16:22:11 CST' for field: " + fieldName);
        }
    ...
    private static ParameterValues convertExistField(String viewField, String funName, Object value) throws InvalidFieldException {
        // check the parameter value
        if (value == null) {
            throw new InvalidFieldException("Value of " + viewField + " hasn't been sepcified.");
        }

        // the value can only be true or false.
        String strValue = value.toString();
        if (!"true".equalsIgnoreCase(strValue) && !"false".equalsIgnoreCase(strValue)) {
            throw new InvalidFieldException("Value of " + viewField + " can only be 'true' or 'false'.");
        }
        ...
        if (funName.equals(QueryOperator.EQUALS)) {
            ...
            throw new InvalidFieldException(viewField + " can only support operators: =, <>, != ");
        }

        return result;
    }

    public static ParameterValues convertTaskStatus(String viewField, String funName, Object value) throws InvalidFieldException {
        ...
        } else if ((QueryOperator.IN.equalsIgnoreCase(funName)) || (QueryOperator.NOT_IN.equalsIgnoreCase(funName))) {
    ...
    public static String getTaskViewField(String field) throws InvalidFieldException {
        String newField = field;
        if (field.startsWith(ATTRIBUTE_PREFIX)) {
            newField = field.substring(ATTRIBUTE_PREFIX.length());
        }
    ...
    public static ParameterValues convertAttachmentField(String viewField, String funName, Object value) {
        String mappingField = FIELDS_MAP.get(viewField);
