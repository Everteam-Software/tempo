...
public class TMSServer implements ITMSServer {
	...
	public TMSServer(IAuthProvider authProvider,
			ITaskDAOConnectionFactory taskDAOFactory,
	...
	public void setAuthProvider(IAuthProvider authProvider) {
		...
		_logger.info("IAuthProvider implementation : "
				+ _authProvider.getClass());
	}
	...
	public Task getTask(String taskID, String participantToken)
		...
		if ((task != null)) {
			...
			if (_logger.isDebugEnabled())
				_logger.debug("Workflow Task " + taskID + " for user "
	...
	private void checkIsAvailable(String taskID, Task task,
			UserRoles credentials) throws AccessDeniedException {
		if (!task.isAvailableTo(credentials))
			throw new AccessDeniedException(credentials.getUserID()
					+ " cannot access task:" + taskID);
	}

	public void setOutput(String taskID, Document output,
		...
		if (task instanceof ITaskWithOutput) {
			...
			dao.commit();
			if (_logger.isDebugEnabled())
				_logger.debug(credentials.getUserID()
						+ " has set output for Workflow Task " + task);
		} else
	...
	public void complete(String taskID, String participantToken)
		...
		if (task instanceof ITaskWithState) {
			...
			if (_logger.isDebugEnabled())
				_logger.debug(credentials.getUserID()
						+ " has completed the Workflow Task " + task);
		} else {
			throw new UnavailableTaskException(credentials.getUserID()
					+ " cannot complete Workflow Task " + task);
		}
	}
	...
	public void setOutputAndComplete(String taskID, Document output,
		...
		if (task instanceof ITaskWithOutput && task instanceof ITaskWithState) {
			...
			if (_logger.isDebugEnabled())
				_logger
						.debug(credentials.getUserID()
								+ " has set output and completed Workflow Task "
								+ task);
		} else {
			throw new UnavailableTaskException(credentials.getUserID()
	...
	public void fail(String taskID, String failureCode, String failureReason,
		...
		if (available) {
			...
			_logger.debug(
			// credentials.getUserID() +
					" fails Workflow Task " + task + " with code: "
	...
	public void delete(String[] taskIDs, String participantToken)
		...
		for (String taskID : taskIDs) {
			try {
				Task task = dao.fetchTaskIfExists(taskID);
				if (_permissions.isAuthorized(TaskPermissions.ACTION_DELETE,
						task, credentials)) {
					dao.deleteTask(task.getInternalId(), taskID);
					dao.commit();
					if (_logger.isDebugEnabled())
						_logger.debug(userID + " has deleted Workflow Task "
								+ task);
				} else {
					problemTasks.put(taskID, new AuthException(userID
							+ " cannot delete" + taskID));
				}
			} catch (Exception e) {
		...
		if (problemTasks.size() > 0) {
			throw new UnavailableTaskException(userID
					+ " cannot delete Workflow Tasks: " + problemTasks.keySet());
		}
	}
	...
	private Document sendInitMessage(PIPATask task, String user,
			String formUrl, String participantToken, Document input)
			throws AxisFault {
		...
		OMElement omUser = omFactory.createOMElement("user", omNamespace,
				omInitProcessRequest);
		omUser.setText(user);

		OMElement omFormUrl = omFactory.createOMElement("formUrl", omNamespace,
				omInitProcessRequest);
		...
		EndpointReference endpointReference = new EndpointReference(task
		...
		if (_logger.isDebugEnabled()) {
			_logger.debug(task
					+ " was used to start the process with endpoint:"
					+ task.getProcessEndpoint());
			_logger
					.debug("Request to Ode:\n"
		...
		try {
			...
			_logger.error("Error while sending initProcessRequest:"
					+ e.getClass(), e);
	...
	public Document initProcess(String taskID, String user, String formUrl,
			Document input, String participantToken) throws AuthException,
		...
		if (task instanceof PIPATask) {
			...
			Document document = sendInitMessage(pipaTask, user, formUrl,
					participantToken, input);
			if (_logger.isDebugEnabled())
				_logger.debug(credentials.getUserID()
						+ " has initialized process "
						+ pipaTask.getProcessEndpoint()
	...
	public Attachment[] getAttachments(String taskID, String participantToken)
		...
		if (task instanceof ITaskWithAttachments) {
			...
			return taskWithAttachments.getAttachments().toArray(
					new Attachment[] {});
		} else {
	...
	public void addAttachment(String taskID, Attachment attachment,
		...
		if (task instanceof ITaskWithAttachments == false) {
			throw new UnavailableTaskException(
					"Task does not support attachments");
		}
		ITaskWithAttachments taskWithAttachments = (ITaskWithAttachments) task;
	...
	public void removeAttachment(String taskID, URL attachmentURL,
		...
		try {
			...
			availableTask = task instanceof ITaskWithAttachments
					&& task.isAvailableTo(credentials);
			if (availableTask) {
				ITaskWithAttachments taskWithAttachments = (ITaskWithAttachments) task;
				Attachment removedAttachment = taskWithAttachments
				...
				if (availableAttachment) {
					...
					if (_logger.isDebugEnabled())
						_logger.debug(credentials.getUserID()
								+ " has removed attachment " + attachmentURL
	...
	public void reassign(String taskID, AuthIdentifierSet users,
		...
		try {
			...
			if (available) {
				...
				dao.commit();
				if (_logger.isDebugEnabled())
					_logger.debug(" changed to user owners " + users
							+ " and role owners " + roles);
			}
		} catch (Exception e) {
			_logger.error("Cannot retrieve Workflow Tasks", e);
		} finally {
	...
	public void deletePipa(String formUrl, String participantToken)
			throws AuthException, UnavailableTaskException {
		try {
			// UserRoles credentials =
	...
	public PIPATask getPipa(String formUrl, String participantToken)
			throws AuthException, UnavailableTaskException {
		try {
			ITaskDAOConnection dao = _taskDAOFactory.openConnection();
	...
	public void storePipa(PIPATask task, String participantToken)
			throws AuthException, InvalidTaskException {
		try {
			ITaskDAOConnection dao = _taskDAOFactory.openConnection();
	...
	public Long countAvailableTasks(String participantToken, HashMap parameters)
			throws AuthException {
		UserRoles credentials = _authProvider.authenticate(participantToken);
		ITaskDAOConnection dao = _taskDAOFactory.openConnection();
		try {
			parameters.put(TaskFetcher.FETCH_USER, credentials);
			String klass = (String) parameters
					.get(TaskFetcher.FETCH_CLASS_NAME);
			if (klass != null)
				parameters.put(TaskFetcher.FETCH_CLASS, Class
						.forName("org.intalio.tempo.workflow.task." + klass));
			return dao.countAvailableTasks(parameters);
		} catch (Exception e) {
			_logger.error("Error while tasks list retrieval for user "
					+ credentials.getUserID(), e);
			throw new RuntimeException(e);
		}
	}

	public Task[] getAvailableTasks(String participantToken, HashMap parameters)
			throws AuthException {
		UserRoles credentials = _authProvider.authenticate(participantToken);
		ITaskDAOConnection dao = _taskDAOFactory.openConnection();
		try {
	...
	public void deleteAll(boolean fakeDelete, String subquery, String taskType,
		...
		for (int i = 0; i < length; i++) {
			_logger.info("Task:" + tasks[i].getClass() + ":" + tasks[i].getID()
	...
	public void skip(String taskID, String participantToken)
			throws AuthException, UnavailableTaskException,
			InvalidTaskStateException, AccessDeniedException {
		UserRoles credentials = _authProvider.authenticate(participantToken);
		ITaskDAOConnection dao = _taskDAOFactory.openConnection();
		...
		if (task instanceof ITaskWithState) {
			...
			if (_logger.isDebugEnabled())
				_logger.debug(credentials.getUserID()
						+ " has skiped the Workflow Task " + task);
		} else {
			throw new UnavailableTaskException(credentials.getUserID()
