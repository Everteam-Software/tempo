...
public class OpenSSORBACFilter implements Filter {
            ...
            try {
                ...
                if (manager.isValidToken(ssoToken)) {
                    ...
                    if (null == session.getAttribute(SESSION_APPLICATION_STATE)) {
                        doSignIn(httpServletRequest, session, ssoToken.getTokenID().toString());
                    }
                    chain.doFilter(request, response);
                }
            } catch (SSOException e) {
                throw new RemoteException("SSOException occured: " + e.getMessage());
            }
    ...
    private void doSignOut(HttpSession session, HttpServletResponse response) throws IOException {
        LOG.info("signing out from Opensso..");
        session.invalidate();
        // manager.destroyToken(ssoToken);

        // redirect to logout url
        String logoutUrl = this._filterConfig.getInitParameter(LOGOUT_URL_FILTER_CFG);
        response.sendRedirect(logoutUrl);
        LOG.info("user has been signed out successfully from Intalio");
    }

    private void doSignIn(HttpServletRequest httpServletRequest, HttpSession session, String ssoTokenId) throws RemoteException {
    ...
    protected User authenticate(TokenService tokenService, String token) throws SecurityException {
        try {
