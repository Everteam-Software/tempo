...
public class CASRBACFilter implements Filter{
    ...
    private static final String JSP_FILE_EXT =".jsp";
    ...
    public void doFilter(ServletRequest request,
        ...
        if( LOGOUT_PATH.equalsIgnoreCase(httpServletRequest.getServletPath())
                && LOGOUT_METHOD.equalsIgnoreCase(httpServletRequest.getMethod())
    ...
    private void doSignIn(HttpServletRequest httpServletRequest, HttpSession session) throws RemoteException {
        ...
        String serviceURL = _filterConfig.getInitParameter("edu.yale.its.tp.cas.client.filter.serviceUrl");
        String tempoConfigPath = System.getProperty(TEMPO_CONFIG_DIRECTORY);
        ApplicationContext appContext = new FileSystemXmlApplicationContext((new StringBuilder("/")).append(tempoConfigPath).append("/").append(SECURITY_CONFIG_XML).toString());
        ...
        if (null != userName && !"".equals(userName)) {
            try {
                LOG.info("signing in for user: " + userName);
                String token = null;
                ...
                ApplicationState.setCurrentInstance(httpServletRequest, state);
                LOG.info("user has been signed in successfully.");
            } catch (RBACException e) {
                e.printStackTrace();
            } catch (AuthenticationException e) {
                e.printStackTrace();
    ...
    private void doSignOut(HttpSession session, HttpServletResponse httpServletResponse) throws IOException {
        ...
        session.invalidate();
        httpServletResponse.sendRedirect("https://localhost:8443/cas/logout");
